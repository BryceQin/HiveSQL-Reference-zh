...
- [1. 内建运算符(Built-in Operators)](#1-内建运算符built-in-operators)
    ...
    - [1.5 String Operators(字符串运算符)](#15-string-operators字符串运算符)
        - [1.5.1 字符串拼接 `||`](#151-字符串拼接-)
    - [1.6 Complex Type Constructors(复杂类型构造符)](#16-complex-type-constructors复杂类型构造符)
        - [1.6.1 字典构造符 `map`](#161-字典构造符-map)
        - [1.6.2 匿名结构体构造符 `struct`](#162-匿名结构体构造符-struct)
        - [1.6.3 命名结构体构造符 `named_struct`](#163-命名结构体构造符-named_struct)
        - [1.6.4 数组构造符 `array`](#164-数组构造符-array)
    - [1.7 Operators on Complex Types(复杂类型操作符)](#17-operators-on-complex-types复杂类型操作符)
        - [1.7.1 数组操作符 `[]`](#171-数组操作符-)
        - [1.7.2 字典操作符 `[]`](#172-字典操作符-)
        - [1.7.3 结构体操作符 `.`](#173-结构体操作符-)
...
- [3. Built-in Aggregate Functions(UDAF)(聚合函数)](#3-built-in-aggregate-functionsudaf聚合函数)
    ...
    - [3.17 回归方程截距函数 `regr_intercept`](#317-回归方程截距函数-regr_intercept)
...
返回值类型：
    struct
说明：
    根据给定的值创建结构体，字段名为col1，col2……。struct取值用`.`
举例：
    SELECT
        struct(1, 'a', 2, 'b')
    -- 结果：
...
返回值类型：
    double
说明：
    返回expr的和。sum(DISTINCT expr)返回expr不重复的和。
举例：
    SELECT
        sum(t1.id1) AS col1,
        sum(DISTINCT t1.id1) AS col2
    FROM
        (
            SELECT
...
返回值类型：
    double
说明：
    返回expr的平均值。avg(DISTINCT expr)返回expr不重复的平均值。null值不参与计算。
举例：
    SELECT
        AVG(t1.id1) AS col1,
        AVG(DISTINCT t1.id1) AS col2
    FROM
        (
            SELECT
                EXPLODE(ARRAY(1, 2, 3, 3, 5, NULL)) AS id1
        ) AS t1
    -- 结果：
    -- col1	col2
    -- 2.8	2.75
...
返回值类型：
    double
说明：
    返回分组中col的总体方差。var_pop(col)是variance(col)的别名。忽略null值。
举例：
    SELECT
        variance(t1.id1) AS col1,
        var_pop(t1.id1) AS col2
    FROM
        (
            SELECT
                EXPLODE(ARRAY(1, 2, 3, 3, 5)) AS id1
...
返回值类型：
    double
说明：
    返回分组中col的样本方差。忽略null值。
举例：
    SELECT
        var_samp(t1.id1) AS col1
    FROM
        (
            SELECT
                EXPLODE(ARRAY(1, 2, 3, 3, 5)) AS id1
...
返回值类型：
    double
说明：
    返回分组中col的总体标准差。忽略null值。
举例：
    SELECT
        stddev_pop(t1.id1) AS col1
    FROM
        (
            SELECT
                EXPLODE(ARRAY(1, 2, 3, 3, 5)) AS id1
...
返回值类型：
    double
说明：
    返回分组中col的样本标准差。忽略null值。
举例：
    SELECT
        stddev_samp(t1.id1) AS col1
    FROM
        (
            SELECT
                EXPLODE(ARRAY(1, 2, 3, 3, 5)) AS id1
...
返回值类型：
    double
说明：
    返回分组中col1和col2的总体协方差。忽略null值。
举例：
    SELECT
        covar_pop(t1.id1, t1.id2) AS col1
    FROM
        (
            SELECT
                EXPLODE(MAP(1, 1, 2, 2, 3, 3, 4, 3, 5, 5)) AS (id1, id2)
        ) AS t1
    -- 结果：
    -- col1
    -- 1.8
```
## 3.11 样本协方差函数 `covar_samp`
```sql
语法：
...
返回值类型：
    double
说明：
    返回分组中col1和col2的样本协方差。忽略null值。
举例：
    SELECT
        covar_samp(t1.id1, t1.id2) AS col1
    FROM
        (
            SELECT
                EXPLODE(MAP(1, 1, 2, 2, 3, 3, 4, 3, 5, 5)) AS (id1, id2)
...
返回值类型：
    double
说明：
    返回分组中col1和col2的皮尔逊相关系数。相当于excel中的CORREL函数。
举例：
    SELECT
        corr(t1.id1, t1.id2) AS col1
    FROM
        (
            SELECT
                EXPLODE(MAP(1, 1, 2, 2, 3, 3, 4, 3, 5, 5)) AS (id1, id2)
...
返回值类型：
    double, array<double>
说明：
    返回分组中col的p分位数。p取值范围为[0,1]。percentile(col, array(p1, p2))返回分组中col的p1和p2分位数。
举例：
    SELECT
        percentile(t1.id1, 0.5) AS col1,
        percentile(t1.id1, array(0.25, 0.5, 0.75)) AS col2
    FROM
        (
            SELECT
                EXPLODE(ARRAY(1, 2, 3, 3, 5)) AS id1
    ...
    -- 3	[2.0,3.0,3.0]
```
## 3.14 近似分位数函数 `percentile_approx`
```sql
语法：
...
返回值类型：
    double, array<double>
说明：
    返回分组中col的近似p分位数。p取值范围为[0,1]。percentile_approx(col, array(p1, p2))返回分组中col的近似p1和p2分位数。B为可选参数，用于控制内存消耗的近似精度，B越大，结果的精确度越高，默认为10000，当col中的distinct元素个数小于B时，结果为准确值。
举例：
    SELECT
        percentile_approx(t1.id1, 0.5, 1) AS col1,
        percentile_approx(t1.id1, array(0.25, 0.5, 0.75)) AS col2
    FROM
        (
            SELECT
                EXPLODE(ARRAY(1.1, 2.1, 3.1, 3.1, 5.1)) AS id1
        ) AS t1
    -- 结果：
    -- col1	col2
    -- 2.9	[1.35,2.35,2.975]
```
## 3.14 自变量平均值函数 `regr_avgx`
```sql
语法：
...
返回值类型：
    double
说明：
    返回分组中dependent的平均值。等价于avg(dependent)。
举例：
    SELECT
        regr_avgx(t1.id1, t1.id2) AS col1,
        avg(t1.id2) AS col2
    FROM
        (
            SELECT
                stack(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5) AS (id1, id2)
        ) AS t1
    -- 结果：
    -- col1	col2
    -- 2.4	2.4
```
## 3.15 因变量平均值函数 `regr_avgy`
```sql
语法：
...
返回值类型：
    double
说明：
    返回分组中independent的平均值。等价于avg(independent)。
举例：
    SELECT
        regr_avgy(t1.id1, t1.id2) AS col1,
        avg(t1.id1) AS col2
    FROM
        (
            SELECT
...
返回值类型：
    double
说明：
    计算independent和dependent都不为空的记录数。
举例：
    SELECT
        regr_count(t1.id1, t1.id2) AS col1
    FROM
        (
            SELECT
                stack(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5) AS (id1, id2)
...
返回值类型：
    double
说明：
    计算由independent, dependent拟合的回归方程的截距。相当于dependent = a * independent + b中b的值。
举例：
    SELECT
        regr_intercept(t1.id1, t1.id2) AS col1
    FROM
        (
            SELECT
                stack(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5) AS (id1, id2)
...
返回值类型：
    double
说明：
    计算由independent, dependent拟合的回归方程的决定系数（也称判定系数/拟合优度/确定系数）。
举例：
    SELECT
        regr_r2(t1.id1, t1.id2) AS col1
    FROM
        (
            SELECT
                stack(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5) AS (id1, id2)
...
返回值类型：
    double
说明：
    计算由independent, dependent拟合的回归方程的斜率。相当于dependent = a * independent + b中a的值。
举例：
    SELECT
        regr_slope(t1.id1, t1.id2) AS col1
    FROM
        (
            SELECT
                stack(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5) AS (id1, id2)
...
返回值类型：
    double
说明：
    计算由independent, dependent拟合的回归方程的自变量离差平方和。等价于regr_count(independent, dependent) * var_pop(dependent)。
举例：
    SELECT
        regr_sxx(t1.id1, t1.id2) AS col1,
        regr_count(t1.id1, t1.id2) * var_pop(t1.id2) AS col2
    FROM
        (
            SELECT
                stack(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5) AS (id1, id2)
...
返回值类型：
    double
说明：
    计算由independent, dependent拟合的回归方程的离差积和。等价于regr_count(independent, dependent) * covar_pop(independent, dependent)。
举例：
    SELECT
        regr_sxy(t1.id1, t1.id2) AS col1,
        regr_count(t1.id1, t1.id2) * covar_pop(t1.id1, t1.id2) AS col2
    FROM
        (
            SELECT
                stack(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5) AS (id1, id2)
...
返回值类型：
    double
说明：
    计算由independent, dependent拟合的回归方程因变量离差平方和。等价于regr_count(independent, dependent) * var_pop(independent)。
举例：
    SELECT
        regr_syy(t1.id1, t1.id2) AS col1,
        regr_count(t1.id1, t1.id2) * var_pop(t1.id1) AS col2
    FROM
        (
            SELECT
                stack(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5) AS (id1, id2)
...
返回值类型：
    array<struct {'x','y'}>
说明：
    直方图表示来自经验数据的频率分布。此处指的是具有可变大小的bin的直方图。具体而言,b代表bin的数量，返回值为一个数组，其中每个元素是一个struct，包含两个字段：x和y。x代表bin的中心值，y代表该bin的高度。
举例：
    SELECT
        histogram_numeric(t1.id1, 3) AS col1
    FROM
        (
            SELECT
                EXPLODE(array(1, 1, 2, 2, 3, 3, 4, 3, 5, 5)) AS id1
        ) AS t1
    -- 结果：
    -- col1
    -- [{"x":1.0,"y":2.0},{"x":2.6,"y":5.0},{"x":4.666666666666667,"y":3.0}]
```
## 3.24 集合去重函数 `collect_set`
```sql
语法：
    collect_set(col)
参数：
...
```sql
语法：
    ntile(x integer) over ([partition_by_clause] <order_by_clause>)
参数：
    x：分桶的数量
返回值类型
    integer
说明：
    通常作为窗口函数使用。将数据按照指定列的值进行分桶，并返回每个分桶的编号。编号从1开始。
举例：
    SELECT
        t1.id1,
        ntile(2) over(order by t1.id1)
    FROM
        (
            SELECT
                EXPLODE(ARRAY(5, 1, 1, 1, 2, 1, 1, 2, 3, 2, 5)) AS id1
        ) AS t1
    -- 结果：
    -- id1	ntile_window_0
    -- 1	1
...
### 4.1.2 explode (map)
```sql
-- 将map中的每个键值对展开为一行，包含key和value两列
SELECT
    explode(MAP('a', 1, 'b', 2, 'c', 3)) AS (key, value)
-- 结果：
-- key	value
-- a	1
-- b	2
...
```sql
-- 将数组中的每个元素展开为一行，并附带元素的位置索引（从0开始）
SELECT
    posexplode(ARRAY('apple', 'banana', 'orange')) AS (pos, fruit)
-- 结果：
-- pos	fruit
-- 0	apple
-- 1	banana
...
-- 将多个值按指定行数堆叠为多行
SELECT
    stack(2, 'Alice', 25, 'Bob', 30) AS (name, age)
-- 结果：
-- name	age
-- Alice	25
-- Bob	30
...
参数类型：
    ...
    m: MAP<K, V>，K和V为任意类型
返回值类型：
    输入为ARRAY时：多行，每行一个列，类型为T
    输入为MAP时：多行，每行两个列，类型分别为K和V
说明：
    将数组或map中的每个元素展开为一行。
    对于数组，输出包含一列，每行为数组中的一个元素。
    对于map，输出包含两列（key和value），每行为map中的一个键值对。
    该函数属于UDTF，使用时需要配合LATERAL VIEW或在SELECT中直接使用。
举例：
    -- 展开数组
    SELECT
        t1.id,
    ...
    FROM
        (
            ...
            LATERAL VIEW explode(t1.arr) t2 AS item
    -- 结果：
    -- id	item
    -- 1	a
    -- 1	b
    ...
    FROM
        (
            ...
            LATERAL VIEW explode(t1.mp) t2 AS key, value
    -- 结果：
    -- id	key	value
    -- 1	x	10
    -- 1	y	20
...
参数类型：
    a: ARRAY<T>，T为任意类型
返回值类型：
    多行，每行两个列：pos为INT类型（元素位置索引，从0开始），col类型为T（元素值）
说明：
    与explode类似，将数组中的每个元素展开为一行，但同时返回元素在数组中的位置索引（从0开始）。
    该函数属于UDTF，使用时需要配合LATERAL VIEW或在SELECT中直接使用。
举例：
    SELECT
        t1.id,
    ...
    FROM
        (
            SELECT
                1 AS id,
                ARRAY('apple', 'banana', 'orange') AS arr
        ) AS t1
            LATERAL VIEW posexplode(t1.arr) t2 AS pos, item
    -- 结果：
    -- id	pos	item
    -- 1	0	apple
    -- 1	1	banana
...
参数类型：
    ...
    k1, k2, ...: string，需要提取的JSON键名，支持多个键
返回值类型：
    多列，每列类型为string，列的顺序与传入键名的顺序一致
说明：
    从给定的JSON字符串中提取多个键对应的值，每个键作为一列输出。
    相比多次使用get_json_object，json_tuple只需解析一次JSON，性能更优。
    该函数属于UDTF，使用时需要配合LATERAL VIEW或在SELECT中直接使用。
举例：
    SELECT
        t1.id,
    ...
    FROM
        (
            SELECT
                1 AS id,
                '{"name":"张三","age":"28","city":"北京"}' AS json_str
        ) AS t1
            LATERAL VIEW json_tuple(t1.json_str, 'name', 'age', 'city') t2 AS name, age, city
    -- 结果：
    -- id	name	age	city
    -- 1	张三	28	北京
```
...
```sql
语法：
    parse_url_tuple(string urlStr, string partToExtract1, string partToExtract2, ...)
参数类型：
    urlStr: string，需要解析的URL字符串
    partToExtract1, partToExtract2, ...: string，需要提取的URL部分标识，可选值包括：
        - HOST：主机名
        - PATH：路径
        - QUERY：查询参数
        - REF：锚点
        ...
        - FILE：文件路径
        - USERINFO：用户信息
        - QUERY:<key>：提取指定查询参数的值
返回值类型：
    多列，每列类型为string，列的顺序与传入部分标识的顺序一致
说明：
    从给定的URL字符串中提取多个部分的值，每个部分作为一列输出。
    该函数遵循URL解析标准，可一次性提取URL的多个组成部分。
    该函数属于UDTF，使用时需要配合LATERAL VIEW或在SELECT中直接使用。
举例：
    -- 提取URL的主机名、路径和指定查询参数
    SELECT
    ...
    FROM
        (
            SELECT
                1 AS id,
                'https://www.example.com/products/detail?id=12345&type=book' AS url_str
        ) AS t1
            LATERAL VIEW parse_url_tuple(t1.url_str, 'HOST', 'PATH', 'QUERY:id', 'PROTOCOL') t2 AS host, path, query_id, protocol
    -- 结果：
    -- id	host	path	query_id	protocol
    -- 1	www.example.com	/products/detail	12345	https
    ...
    FROM
        (
            SELECT
                1 AS id,
                'http://user:pass@host.com:8080/p/a/t/h?query=string#hash' AS url_str
        ) AS t1
            LATERAL VIEW parse_url_tuple(t1.url_str, 'QUERY', 'REF') t2 AS full_query, ref
    -- 结果：
    -- id	full_query	ref
    -- 1	query=string	hash
```
