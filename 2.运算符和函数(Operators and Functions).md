...
- [3. Built-in Aggregate Functions(UDAF)(聚合函数)](#3-built-in-aggregate-functionsudaf聚合函数)
    ...
    - [3.24 集合去重函数 `collect_set`（待修改）](#324-集合去重函数-collect_set待修改)
...
举例：
    ...
    FROM
        (
            SELECT
                EXPLODE(MAP(1, 1, 2, 2, 3, 3, 4, 3, 5,null)) AS (id1, id2)
        ) AS t1
    -- 结果：
    -- 4
```
...
举例：
    ...
    FROM
        (
            SELECT
                EXPLODE(MAP(1, 2, 2, 3, 2, 3, null, 4, 4,null)) AS (id1, id2)
        ) AS t1
    -- 结果：
    -- 1
```
...
举例：
    ...
    FROM
        (
            SELECT
                EXPLODE(MAP(1, 1, 1, 2, 1, 1, 2, 3, 2, 5)) AS (id1, id2)
        ) AS t1
    GROUP BY
        t1.id1
    -- 结果：
